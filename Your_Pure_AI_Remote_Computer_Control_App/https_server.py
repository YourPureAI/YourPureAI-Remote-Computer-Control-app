...
import re # Import regular expressions for variable substitution
import urllib.parse
...
MANAGEMENT_HTML_FILE = "manage_scenarios.html" # HTML file for management UI
...
def substitute_variables(template_string, variables):
    """Substitutes placeholders like ${var_name} or ${enc_var_name} in the template_string."""
    if not variables: # Zpracování případu, kdy dataForExecution chybí nebo je prázdné
        variables = {}

    def replace_match(match):
        placeholder = match.group(0) # Celý zástupný symbol, např. ${my_var} nebo ${ enc_test }
        var_name = match.group(1)    # Jméno proměnné, např. my_var nebo enc_test

        # Zkontrolujte, zda proměnná existuje v poskytnutém slovníku
        if var_name in variables:
            value = variables[var_name]
            # Zkontrolujte, zda jméno proměnné začíná na 'enc_'
            if var_name.startswith("enc_"):
                # URL-enkódujte hodnotu. Bezpečnostní parametr safe='' zajistí enkódování i znaků jako '/'
                # Nejprve převeďte hodnotu na řetězec pro jistotu
                encoded_value = urllib.parse.quote(str(value), safe='')
                print(f"Encoding variable '{var_name}' value '{str(value)}' to '{encoded_value}'") # Logování pro ladění
                return encoded_value
            else:
                # Vraťte hodnotu jako řetězec bez enkódování
                return str(value)
        else:
            # Proměnná nebyla nalezena ve slovníku, vraťte původní zástupný symbol
            print(f"Varování: Proměnná {placeholder} nebyla nalezena v dataForExecution.")
            return placeholder # Vrací např. ${neznama_promenna}

    # Regex pro nalezení ${variable_name} - umožňuje mezery kolem jména
...
def execute_scenario_command(scenario, data_for_execution):
    ...
    if command_type == "cmd":
        ...
        # Using /d to disable AutoRun commands from registry
        # Using /c to carry out the command specified by string and then terminate
        ...
        if current_os == "Windows":
            ...
            print(f"Execute Scenario: Warning - {msg}")
            # Consider WSL if needed in the future, but for now, return error
            return False, msg
        shell_name = "Linux Shell (/bin/sh or similar)"
        # Using default shell for execution
        args = ['/bin/sh', '-c', command_to_execute]
        # Alternative: Let subprocess figure out the default shell
        # args = command_to_execute # Requires shell=True which is a security risk if not careful
    else:
    ...
    try:
        ...
        # IMPORTANT SECURITY NOTE: Passing the fully substituted command_to_execute
        # directly to cmd /c or powershell -Command can be risky if the variables
        # in data_for_execution contain shell metacharacters.
        # Consider more robust parsing or sanitization for production use.
        result = subprocess.run(
            args,
            capture_output=True,
            text=True,
            check=False, # Don't raise exception on non-zero exit code, just report it
            encoding='utf-8',
            errors='replace',
            startupinfo=startupinfo,
            shell=False # Crucial for security when args is a list
        )
        ...
        # Return success status based on return code
...
class APIHandler(http.server.SimpleHTTPRequestHandler):
    ...
    # Helper to send plain text/html responses
    ...
    # Override send_error for UTF-8
    def send_error_response(self, code, message=None):
        """Managing UTF-8 for error responses."""
        ...
        print(f"Sent error response: {error_message}") # Log errors


    def serve_management_page(self):
        """Serves the HTML file for managing scenarios."""
        if not os.path.exists(MANAGEMENT_HTML_FILE):
            ...
            return
        try:
            with open(MANAGEMENT_HTML_FILE, 'rb') as f: # Read as bytes
                content = f.read()
    ...
    def do_GET(self):
        """Handles GET requests for the management UI and scenario listing."""
        config = load_config() # Load config for API key check if needed later
    ...
    def do_POST(self):
        """Handles POST requests for API actions and scenario management."""
        config = load_config()
        expected_api_key = config.get("api_key", "MISSING_API_KEY") # Use default if missing
        ...
        # --- API Key Check ---
        # Check API key for ALL POST requests (API and Management) for security
        # Allow missing key only if it's the default/initial one for easier setup
        ...
        try:
            if path == '/api':
                ...
                data_for_execution = json_data.get('dataForExecution', {}) # Default to empty dict
                ...
                if found_scenario and found_scenario.get('allowed', False):
                    ...
                    # Execute the command
                    success, message = execute_scenario_command(found_scenario, data_for_execution)
                    if success:
                        ...
                        # Send error, but maybe not 500 unless it's a server fault
                        self.send_json_response(HTTPStatus.INTERNAL_SERVER_ERROR, response_data)

                else:
                    if found_scenario: # Found but not allowed
                    ...
                    # Original behavior: Save request to file (excluding API key)
                 ...
                 # --- Handle Saving All Scenarios (from Management UI) ---
                 # Expecting the entire list of scenarios in the payload's 'scenarios' key
                 ...
                 for i, scenario in enumerate(new_scenarios_list):
                      if not isinstance(scenario, dict) or 'name' not in scenario or 'command_type' not in scenario or 'command_template' not in scenario or 'allowed' not in scenario:
                           ...
                           return
                      # Ensure name is unique? (Optional, but good practice)
            ...
            print(error_message)
            traceback.print_exc()
            # Avoid sending detailed tracebacks to the client for security
...
def run_server():
    ...
    # Create the management HTML file if it doesn't exist
    if not os.path.exists(MANAGEMENT_HTML_FILE):
        create_default_management_html()

    handler = APIHandler
    # Allow connections from any interface (0.0.0.0)
    try:
...
def create_default_management_html():
    """Creates a basic manage_scenarios.html if it doesn't exist."""
    html_content = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
...
if __name__ == "__main__":
    try:
        # Ensure necessary files exist or are created before starting server
        load_config() # Create default config if needed
        load_scenarios() # Create default scenarios file if needed
        # create_default_management_html() # Moved check/creation to run_server
